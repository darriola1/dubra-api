...
export const RegisterUserSchema = z.object({
  ...
  confirmPassword: z
    .string()
    .min(6, 'Confirmation is required')
    .openapi({ example: 'StrongP@ss1', description: 'Repeat of the password' }),

  rut: z
    .string()
    .min(12, 'RUT is required, minimum 12 characters')
    .openapi({ example: '12.345.678-9', description: 'Chilean RUT' }),

  recaptchaToken: z
    .string()
    .min(1, 'reCAPTCHA token is missing')
    .openapi({ example: 'recaptcha-token-example', description: 'reCAPTCHA verification token' }),
...
export const LoginUserSchema = z.object({
  email: z
    ...
    .email('Invalid email address')
    .openapi({ example: 'juan@example.com', description: 'User email' }),

  password: z
    .string()
    .min(6, 'Password must be at least 6 characters long')
    .openapi({ example: 'StrongP@ss1', description: 'User password' }),

  recaptchaToken: z
    .string()
    .min(1, 'reCAPTCHA token is missing')
    .openapi({ example: 'recaptcha-token-example', description: 'reCAPTCHA token' }),
});
...
export const ChangePasswordSchema = z.object({
  password: z
    ...
    .openapi({ example: 'OldP@ss1', description: 'Current password' }),

  newPassword: z
    .string()
    .min(6, 'New password must be at least 6 characters long')
    .openapi({ example: 'NewP@ss1', description: 'New password' }),
});
